...
\vspace{5cm}
\begin{center}
{\LARGE \textbf{Essay about the paper \\"A computational model of visual analogies in design" by Davies, Goel \& Nersessian}}
\end{center}
\vspace{7,5cm}
...
Apart from that, the thesis gives us a good idea how these transfer processes could look like. During the course of the paper the researcher develop a computational model which provides a working and comprehensible approach that makes it possible to claim that visual and spatial thinking is useful for many subtasks of analogue problem solving. Also that sequential decomposition and processing of knowledge states of a knowledge source by analogue transfer to a new problem is highly sufficient to solve this problem. As a result, this work and the designed models might contribute for upcoming work within the research for cognition design. As an potential outlook it would be exciting to see how further conclusions can be drawn on the basis of this work. For example one possibility could be to design a new experiment on the topic, and to collect new human output data on solving problems by using analogies. This would also be probably very valuable for other researchers in this area as well. One way would be to conduct a controlled experiment with equally trained and randomly assigned subjects. At the same time, the development of Galatea and Covlan will probably be  more advanced, so that it will be possible to develop an even more precise analysis to verify the results. This way we will gradually better understand how we humans can approach analogy problem solving and design even better suitable models. Once we have understood this, we may even be able to design models that recognize and extract the analogical knowledge on their own and then apply it to completely new problems that we have not thought of ourselves to create something new and valuable for mankind.
